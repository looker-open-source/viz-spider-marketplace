...
function RadarChart(
  ...
  let cfg = {
    ...
    negativeR: 0.81, //scalar dictating negative axis length
    ...
    legendFont: 0.8, //font of legend items
    domainMax: null,
    labelScale: true,
    labelFine: 1.2,
  };
  ...
  if (cfg.independent) {
    ...
    allAxis.map(function (d) {
      rScale.push(scaleLinear().range([0, radius]).domain([0, axesMax[d]]));
    });
    maxValue = [];
    allAxis.map(function (d) {
    ...
    allAxis.map(function (d) {
      rScale.push(scaleLinear().range([0, radius]).domain([0, maxValue]));
    });
    maxValue = [];
    allAxis.map(function (d) {
  ...
  axis
    ...
    .attr("x1", function (d, i) {
      if (cfg.roundStrokes) {
        if (cfg.negatives) {
          return (
            rScale[i](maxValue[i] * -1) * Math.cos(angleSlice * i - Math.PI / 2)
          );
        } else {
          return 0;
        }
    ...
    .attr("y1", function (d, i) {
      if (cfg.roundStrokes) {
        if (cfg.negatives) {
          return (
            rScale[i](maxValue[i] * -1) * Math.sin(angleSlice * i - Math.PI / 2)
          );
        } else {
          return 0;
        }
    ...
    .attr("x2", function (d, i) {
      return (
        rScale[i](maxValue[i] * 1.0) * Math.cos(angleSlice * i - Math.PI / 2)
      );
    })
    .attr("y2", function (d, i) {
      return (
        rScale[i](maxValue[i] * 1.0) * Math.sin(angleSlice * i - Math.PI / 2)
      );
    })
    .attr("class", "line")
  ...
  axis
    ...
    .attr("x", function (d, i) {
      return (
        rScale[i](maxValue[i] * cfg.labelFactor) *
        Math.cos(angleSlice * i - Math.PI / 2)
      );
    })
    .attr("y", function (d, i) {
  ...
  function toggleDataPoints(colorClass) {
    selectAll(`#v${colorClass}`).classed("hidden", function () {
      // toggle "hidden" class
